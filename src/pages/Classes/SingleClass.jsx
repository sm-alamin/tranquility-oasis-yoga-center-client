...
const SingleClass = ({ classItem }) => {
  const { class_name, instructor_name,available_seats,image, price, _id } = classItem;
    ...
    const handleAddToCart = classItem => {
        ...
        if(user && user.email){
            const cartItem = {classItemId: _id, class_name,instructor_name,available_seats, image, price, email: user.email}
           ...
           toast.error('Please! login first').then((result) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6 relative border">
      ...
      <div className="flex w-full justify-between bg-lime-100 p-2">
        ...
        <button onClick={() => handleAddToCart(classItem)} className="btn-primary text-white rounded-lg p-2">
          Enroll Now
        </button>
        </div>
      </div>
